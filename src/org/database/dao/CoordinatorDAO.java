...
public class CoordinatorDAO implements ICoordinatorDAO{
    ...
    public List<Coordinator> getAllCoordinatorsByCourseID(int idCourse) {
        ...
        try(Connection conn = database.getConnection() ){
            conn.setAutoCommit(false);
            String statement = "SELECT CORD.id_person, CORD.cubicle, CORD.staff_number, PERSCORD.name, PERSCORD.phoneNumber, PERSCORD.email, COUR.id_course, COUR.NRC, COUR.period, COUR.name FROM Coordinator AS CORD INNER JOIN PERSON AS PERSCORD ON PERSCORD.id_person = CORD.id_person INNER JOIN COURSE AS COUR ON PERSCORD.id_course = COUR.id_course AND COUR.id_course = ?";
